...
width, height = 1920, 1080   # Projector display size (Second Monitor)
...
def capture_hand_landmarks():
    ...
    for i, point in enumerate(target_points):
        while True:
            ...
            cv2.putText(calibration_image, f'Point {i+1}', (point[0] + 30, point[1] - 30), 
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)

            primary_screen_width = 2560+1920  # Adjust this based on your setup
            window_name = "Calibration Targets"
            cv2.namedWindow(window_name, cv2.WND_PROP_FULLSCREEN)
            cv2.setWindowProperty(window_name, cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
            
            # Move window to the second display (offset X by primary display width)
            cv2.moveWindow(window_name, primary_screen_width, 0)
